"""Forecast scoring utilities used by the /forecast route."""
from __future__ import annotations
from typing import List, Optional, Tuple
import numpy as np

def _ewma(series: List[float], alpha: float) -> float:
    """Return the last EWMA value for a numeric series."""
    v = None
    for x in series:
...
def _clamp_0_100(x: float) -> float:
    """Clamp a value to the [0, 100] interval."""
...
def forecast_next7(
    ...
    """Small, interpretable forecast for the next 7 days.

    - Baseline: EWMA of the last up-to-14 scores.
    - Momentum: mean(last7) - mean(first7), blended across the week.
    - Deadlines: add (deadline_weight * count) per day if provided.
    - Confidence: from std of the last 7 scores, clamped to [5, 20].
    """
